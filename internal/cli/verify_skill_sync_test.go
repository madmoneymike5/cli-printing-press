...
func TestVerifySkillDriftWorkflowGuardsLibraryCopy(t *testing.T) {
	...
	required := []string{
		...
		"gh issue list",
		"gh issue create",
		"op=gh_api repo=$GITHUB_REPOSITORY error=",
		"op=gh_issue_list repo=$GITHUB_REPOSITORY title=$issue_title error=",
		"op=gh_issue_create repo=$GITHUB_REPOSITORY title=$issue_title error=",
